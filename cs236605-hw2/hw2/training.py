...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            epoch_res_train = self.train_epoch(dl_train, verbose=verbose, **kw)
            epoch_res_test = self.test_epoch(dl_test, verbose=verbose, **kw)
            
            train_losses_mean = sum(epoch_res_train.losses) / len(epoch_res_train.losses)
            test_losses_mean = sum(epoch_res_test.losses) / len(epoch_res_test.losses)
            actual_num_epochs += 1
...
class BlocksTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        out = self.model(X)
        loss = self.loss_fn(out, y=y).item()
