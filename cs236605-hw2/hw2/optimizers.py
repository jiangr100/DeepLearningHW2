...
class VanillaSGD(Optimizer):
    ...
    def step(self):
        for p, dp in self.params:
            ...
            p -= self.learn_rate*(dp + self.reg*p)    #use -= to update the actual data
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            if self.velocity[i] is not None:
                self.velocity[i] = self.momentum*self.velocity[i] - self.learn_rate*(dp + self.reg*p)
            else:
                self.velocity[i] = -self.learn_rate*(dp + self.reg*p)
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            if self.lr[i] is not None:
                self.lr[i] = self.decay*self.lr[i] + (1 - self.decay) * ((dp + self.reg*p)**2)
            else:
                self.lr[i] = (1 - self.decay) * ((dp + self.reg*p)**2)
                
            p -= self.learn_rate * (dp + self.reg*p) / torch.sqrt(self.lr[i] + self.eps)
